    ...
    property color dotIndicator: {
        if (displayMode === Dock.Efficient) {
            if (colorTheme === Dock.Dark) {
                return itemActive ? Qt.rgba(1, 1, 1, 0.6) : Qt.rgba(1, 1, 1, 0.3)
            } else {
                return itemActive ? Qt.rgba(0, 0, 0, 0.8) : Qt.rgba(0, 0, 0, 0.3)
            }
        } else if (displayMode === Dock.Fashion) {
            if (colorTheme === Dock.Dark) {
                return itemActive ? backgroundColor : Qt.rgba(1, 1, 1)
            } else {
                return itemActive ? backgroundColor : Qt.rgba(0, 0, 0)
            }
        } else {
            return "#00000000"
        }
