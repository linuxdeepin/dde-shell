...
{
    if (!m_hoverPreviewModel) {
        qWarning() << "TaskManager::requestPreview: m_hoverPreviewModel is null";
        return;
    }
    ...
    if (m_hoverPreviewModel->rowCount() == 0) {
        qDebug() << "TaskManager::requestPreview: No windows found for index";
        hideItemPreview();
        return;
    }
...
{
    // 不立即清理预览代理模型的状态，让预览窗口的延迟隐藏机制来处理
    // 这样用户可以将鼠标移动到预览窗口上
