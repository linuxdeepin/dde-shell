...
{
public:
    DockItemWindowModel(QObject *parent = nullptr)
    ...
    , m_direction(0)
{
    ...
    connect(m_closeAllButton, &DIconButton::clicked, this, [this]() {
        qDebug() << "closeAllButton clicked";
        // QList<uint32_t> windowIds;
        // for (int i = 0; i < m_model->rowCount(); i++) {
        //     windowIds.append(m_model->index(i).data(WindowIdRole).toInt());
        // }
...
void X11WindowPreviewContainer::showPreview(const QModelIndexList &indexes,
                                            ...
                                            uint32_t direction)
{
    ...
    const QModelIndex &firstIndex = m_model->index(0);
    updatePreviewIconFromBase64(m_model->data(firstIndex, WindowIconRole).toString());
    updatePreviewTitle(m_model->data(firstIndex, WindowTitleRole).toString());
