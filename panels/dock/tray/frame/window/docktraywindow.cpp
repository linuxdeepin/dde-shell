...
Position DockTrayWindow::position() const
{
    return m_position;
}
...
QSize DockTrayWindow::suitableSize(const Dock::Position &position, const int &, const double &) const
{
    ...
    if (position == Dock::Position::Left || position == Dock::Position::Right) {
        ...
        size.setHeight(m_model->rowCount() * 40 + m_dateTimeWidget->suitableSize().width() + m_quickIconWidget->suitableSize().width());
    ...
    size.setWidth(m_model->rowCount() * 40 + m_dateTimeWidget->suitableSize().width() + m_quickIconWidget->suitableSize().width());
...
int DockTrayWindow::dockSize() const
{
    return m_dockSize;
}

void DockTrayWindow::setDockSize(int dockSize)
{
...
void DockTrayWindow::resizeTool() const
{
    ...
    if (m_position == Dock::Position::Left || m_position == Dock::Position::Right)
        m_toolWidget->setFixedSize(40, toolSize);
    else
        m_toolWidget->setFixedSize(toolSize, 40);
}
...
void DockTrayWindow::updateToolWidget()
{
    ...
    m_toolFrontSpaceWidget->setVisible(false/*m_toolLayout->count() > 0*/);
