...
QSize SystemPluginWindow::suitableSize(const Position &position) const
{
    ...
    if (position == Dock::Position::Top || position == Dock::Position::Bottom) {
        int itemWidth = 0;
        for (StretchPluginsItem *item : items)
            itemWidth += /*item->suitableSize(position).width()*/30;
    ...
    int itemHeight = 0;
    for (StretchPluginsItem *item : items) {
        itemHeight += /*item->suitableSize(position).height()*/30;
    }

    return QSize(30, itemHeight);
}

void SystemPluginWindow::paintEvent(QPaintEvent *event)
{
...
void StretchPluginsItem::paintEvent(QPaintEvent *event)
{
    ...
    painter.drawPixmap(rctPixmap, pixmap);
}
...
void StretchPluginsItem::enterEvent(QEnterEvent *event)
{
