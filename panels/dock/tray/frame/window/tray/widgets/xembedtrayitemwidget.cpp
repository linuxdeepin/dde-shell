...
void XEmbedTrayItemWidget::showEvent(QShowEvent *e)
{
    ...
    xcb_connection_t *connection = IS_WAYLAND_DISPLAY ? m_xcbCnn : /*QX11Info::connection()*/qGuiApp->nativeInterface<QNativeInterface::QX11Application>()->connection();
    if (connection) {
...
{
    auto c = IS_WAYLAND_DISPLAY ? m_xcbCnn : qGuiApp->nativeInterface<QNativeInterface::QX11Application>()->connection();
    if (!c) {
        qWarning() << "x11 connection() is " << c;
        return;
    }
...
void XEmbedTrayItemWidget::sendHoverEvent()
{
    ...
    setWindowOnTop(true);
    Display *display = IS_WAYLAND_DISPLAY ? m_display : qGuiApp->nativeInterface<QNativeInterface::QX11Application>()->display();
    if (display) {
...
void XEmbedTrayItemWidget::setX11PassMouseEvent(const bool pass)
{
    ...
    const auto display = qGuiApp->nativeInterface<QNativeInterface::QX11Application>()->display();

    if (pass)
    {
        XShapeCombineRectangles(display, m_containerWid, ShapeBounding, 0, 0, nullptr, 0, ShapeSet, YXBanded);
        XShapeCombineRectangles(display, m_containerWid, ShapeInput, 0, 0, nullptr, 0, ShapeSet, YXBanded);
    }
    else
    {
        ...
        XShapeCombineRectangles(display, m_containerWid, ShapeBounding, 0, 0, &rectangle, 1, ShapeSet, YXBanded);
        XShapeCombineRectangles(display, m_containerWid, ShapeInput, 0, 0, &rectangle, 1, ShapeSet, YXBanded);
    }
...
{
    const auto display = IS_WAYLAND_DISPLAY ? XOpenDisplay(nullptr) : qGuiApp->nativeInterface<QNativeInterface::QX11Application>()->display();
    if (!display) {
        qWarning() << "x11 connection() is " << display;
        return 0;
    }
