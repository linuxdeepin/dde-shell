...
void AppItem::updateWindowIconGeometries()
{
    ...
    if (!x11App) {
        qWarning() << "x11 connection() is 0x0";
        return;
    }
...
void AppItem::mouseReleaseEvent(QMouseEvent *e)
{
    ...
    if (e->button() == Qt::MiddleButton) {
        m_itemEntry->newInstance(/*QX11Info::getTimestamp()*/0);
        ...
        if (m_showMultiWindow) {
            ...
            // 如果没有开启新窗口显示，则
            m_itemEntry->active(/*QX11Info::getTimestamp()*/0);
            // play launch effect
...
void AppItem::dropEvent(QDropEvent *e)
{
    ...
    m_itemEntry->handleDragDrop(/*QX11Info::getTimestamp()*/0, uriList);
}
...
void AppItem::invokedMenuItem(const QString &itemId, const bool checked)
{
    ...
    m_itemEntry->handleMenuItem(/*QX11Info::getTimestamp()*/0, itemId);
}
