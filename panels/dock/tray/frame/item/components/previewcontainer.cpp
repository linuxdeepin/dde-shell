...
void PreviewContainer::adjustSize(bool composite)
{
    ...
    auto screen = qApp->screenAt(this->geometry().center());
    const int screenWidth = /*QDesktopWidget().screenGeometry(this)*/screen->geometry().width();
    const int screenHeight = /*QDesktopWidget().screenGeometry(this)*/screen->geometry().height();
